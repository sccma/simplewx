...
if ($_W['isw7_request'] && !$_W['isadmin']) {
	...
	if (isset($_GPC['w7_accesstoken'])) {
		...
		if (empty($_W['uid'])) {
			...
			WeUtility::logging('auth-verify-data', array('secret' => $_GPC['w7_request_secret'], 'remote_secret' => $onlineSecret, 'accesstoken' => $_GPC['w7_accesstoken'], 'request_token' => $request_token), true);
			iajax(403, '身份验证失败，请联系官方客服处理！');
		}
	}
}
