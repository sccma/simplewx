	...
	<form action="" method="get">
		<table class="table we7-table table-hover vertical-middle table-manage">
			...
			<tr>
				...
				<th class="text-left bg-light-gray">维护状态</th>
				<th></th>
			...
			<tr ng-repeat="(key, module) in moduleList"  ng-if="key >= (pageindex - 1) * pagesize && key < pageindex * pagesize">
				...
				<td class="text-left">
					<p>
						{{ module.title }}
						<i class="wi wi-info color-red" data-toggle="popover" data-trigger="hover" data-container="body" ng-if="module.system_shutdown == 2 && !module.can_update" data-placement="bottom" data-content="该应用因违反平台规定已于<span class='color-red'>{{module.system_shutdown_delay_time}}</span>下架，无法购买和更新版本" data-html="true"></i>

						<i class="wi wi-info color-red" data-toggle="popover" data-trigger="hover" data-container="body" ng-if="module.system_shutdown == 2 && module.can_update" data-placement="bottom" data-content="该应用因违反平台规定已于<span class='color-red'>{{module.system_shutdown_delay_time}}</span>下架，无法购买但可以更新版本，一个月后将无法更新，如有疑问请及时反馈" data-html="true"></i>

						<i class="wi wi-info color-red" data-toggle="popover" data-trigger="hover" data-container="body" ng-if="module.system_shutdown == 3 && module.can_update" data-placement="bottom" data-content="该应用因违反平台规定将于<span class='color-red'>{{module.system_shutdown_delay_time}}</span>下架，目前不影响版本升级，<span class='color-red'>下架后将无法购买和升级版本</span>，如有疑问请及时反馈" data-html="true"></i>
				...
				<td class="text-left">
					<span class="color-green" ng-if="module.status == 1 || module.from == 'local'">-</span>
					<span class="color-red" ng-if="module.status == 0 && module.from != 'local'">
						停止维护
						<i class="wi wi-info color-red" data-toggle="popover" data-trigger="hover" data-container="body" data-placement="bottom" data-content="该应用或当前购买的分支在<span class='color-red'>应用市场下架后</span>，应用在系统内则处于停止维护状态，此时若应用服务费到期，系统将<span class='color-red'>不限制该应用分配给新的账号进行使用</span>。" data-html="true"></i>
					</span>
				</td>
				<td >
					<!--tag start-->
...
<div id="js-system-module-not_installed" ng-controller="notInstalledCtrl" ng-cloak>
	<div class="search-box clearfix we7-margin-bottom">
		...
		<select class="we7-select we7-margin-right" ng-model="order" ng-change="searchOrder(order)" ng-init="order = '<?php echo $_GPC['order']?$_GPC['order']:''?>'">
			<option value="" ng-selected="order == ''">按购买时间排序</option>
			<option value="buytime_asc" ng-selected="order == 'buytime_asc'">时间正序</option>
			<option value="buytime_desc" ng-selected="order == 'buytime_desc'">时间倒序</option>
		</select>
		...
		<?php  if(!empty($_W['setting']['site']['is_test'])) { ?><a class="btn btn-primary" data-toggle="modal" data-target="#host">测试码安装应用</a><?php  } ?>
	</div>
		...
		<div class="we7-modal-dialog modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<div class="modal-title">测试码安装应用</div>
				</div>
				<div class="modal-body">
					<div class="form-group we7-form">
						<div class="form-group">
							<label class="col-sm-2 control-label"> 输入测试码</label>
							<div class="col-sm-10">
								<input type="text" name="code" class="form-control">
								<span class="help-block">请前去<span class="color-red">开发者中心>设置>测试功能>测试码>添加</span>，选择测试应用后，获取测试码</span>
							</div>
						</div>
					</div>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-primary" ng-click="testCodeInstall()">确定</button>
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<?php  if(!empty($_W['setting']['site']['is_test'])) { ?><a class="btn btn-primary pull-right" data-toggle="modal" data-target="#hostDetail" ng-show="show == 'upgrade'" ng-if="'cloud_test' == moduleinfo.from">测试码升级应用</a><?php  } ?>
	<div class="clearfix">	</div>
	<div class="modal fade" id="hostDetail" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="we7-modal-dialog modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
					<div class="modal-title">测试码升级应用</div>
				</div>
				<div class="modal-body">
					<div class="form-group we7-form">
						<div class="form-group">
							<label class="col-sm-2 control-label"> 输入测试码</label>
							<div class="col-sm-10">
								<input type="text" name="code_upgrade" class="form-control">
								<span class="help-block">请前去<span class="color-red">开发者中心>设置>测试功能>测试码>添加</span>，选择测试应用后，获取测试码</span>
							</div>
						</div>
					</div>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-primary" ng-click="testCodeUpgrade()">确定</button>
					<button type="button" class="btn btn-default" data-dismiss="modal" ng-click="recover()">取消</button>
				</div>
			</div>
		</div>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && localUpgradeInfo">
		<col width="300px"/>
		<col />
		<col width="200px;"/>
		<tr>
...
<script>
	require(['fileUploader'], function() {
		angular.module('moduleApp').value('config', {
			...
			'localUpgradeInfo' : <?php  echo json_encode($local_upgrade_info)?>,
...
<div>
	<table class="table we7-table table-hover vertical-middle table-manage">
		...
		<tr>
			<td class="text-left module-img table-action-td"><img class="module-img" src="<?php  echo ($_W['siteroot'] .''. $item['logo'])?>"/></td>
			<td class="text-left">
				<p><?php  echo $item['title'];?>（<?php  echo $item['name'];?>）</p>
				<span>版本：<?php  echo $item['version'];?> </span>
			</td>
			<td class="text-left">
				<?php  if('cloud' == $item['from']) { ?><span>商城</span><?php  } else { ?><span>本地</span><?php  } ?>
			</td>
		</tr>
		<?php  } } ?>
	</table>
